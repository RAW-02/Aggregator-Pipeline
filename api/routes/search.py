...
@router.get("/search")
def search(query: str, page: int = 1, size: int = 20, sort: str | None = None):
...
def search_with_filters(self, filters, page=1, size=20):
    ...
    if filters.get("kev") is not None:
        must.append({"term": {"kev_status": filters["kev"]}})

    if filters.get("exploit") is not None:
        must.append({"term": {"exploit_available": filters["exploit"]}})

    if filters.get("cwe"):
        must.append({"match": {"cwe": filters["cwe"]}})
