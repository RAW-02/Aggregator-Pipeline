...
def export_data(query: str, format: str = "json"):
    ...
    result = resolver.resolve(query=query, page=1, size=1000)
    ...
    if format.lower() == "csv":
        ...
        writer.writerow(
            ["cve_id", "severity", "cvss_score", "epss_score", "threat_score"]
        )
...
def export_excel(query: str):
    ...
    sheet.append(["CVE ID", "Severity", "CVSS", "EPSS", "Threat Score"])
...
def export_pdf(query: str):
    ...
    elements.append(Paragraph("Vulnerability Report", styles["Title"]))

    elements.append(Paragraph(f"Query: {query}", styles["Normal"]))

    elements.append(Paragraph(f"Total Results: {len(hits)}", styles["Normal"]))

    elements.append(Spacer(1, 12))

    table_data = [["CVE ID", "Severity", "CVSS", "EPSS", "Threat"]]
