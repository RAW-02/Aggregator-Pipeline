...
class JsonRepository(VulnerabilityRepository):
    ...
    def upsert(self, record):
        ...
        for source in ["nvd", "epss", "kev", "exploitdb", "github"]:
    ...
    def get_all(self):
        ...
        for year in os.listdir(self.base_directory):
            year_path = os.path.join(self.base_directory, year)
    ...
    def search(self, keyword):
        ...
        for record in self.get_all():
            if (keyword in record["cve_id"].lower() or keyword in record["description"].lower() or any(keyword in product.lower() for product in record.get("products", []))):
    ...
    def get_pending(self, source, limit=100):
        ids = sorted(self.pending.load(source))
        records = []
