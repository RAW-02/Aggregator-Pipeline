...
class OpenSearchRepository(VulnerabilityRepository):
    ...
    def __init__(self):
        ...
        self.index_name = os.getenv("OPENSEARCH_INDEX", "vulnerabilities")
    ...
    def upsert(self, record):
        ...
        return self.client.index(index=self.index_name, id=cve_id, document=document)
    ...
    def bulk_upsert(self, records):
        ...
        for record in records:
            ...
            actions.append(
                {"_index": self.index_name, "_id": cve_id, "_source": document}
            )
    ...
    def get(self, cve_id):
        try:
            result = self.client.get(index=self.index_name, id=cve_id)
    ...
    def delete(self, cve_id):
        try:
            self.client.delete(index=self.index_name, id=cve_id)
        except Exception as e:
            print(e)
            pass
    ...
    def update_fields(self, cve_id, updates):
        self.client.update(index=self.index_name, id=cve_id, body={"doc": updates})

    def get_all_cve_ids(self):
        query = {"_source": ["cve_id"], "query": {"match_all": {}}}
        ...
        while True:
            ...
            result = self.client.search(
                index=self.index_name, body=query, size=1000, sort=["cve_id"]
            )
