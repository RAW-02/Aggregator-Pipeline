...
class NVDCollector(BaseCollector):
    ...
    def fetch_by_id(self, cve_id):
        ...
        def request():
            ...
            response = HttpClient.session().get(
                self.NVD_BASE_URL,
                headers=headers,
                params={"cveId": cve_id},
                timeout=int(os.getenv("NVD_TIMEOUT", 60)),
            )
        ...
        return self.normalizer.get_result(vulnerabilities[0])
    ...
    def fetch_incremental(self, last_sync):
        ...
        response = RetryManager.execute(
            lambda: HttpClient.session().get(
                self.NVD_BASE_URL, headers=headers, params=params, timeout=60
            )
        )
